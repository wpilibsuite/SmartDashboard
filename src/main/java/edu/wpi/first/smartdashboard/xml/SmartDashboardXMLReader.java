...
public class SmartDashboardXMLReader {
   ...
   */
  private final Map<XMLWidget, Map<Integer, XMLWidget>> subsystems
      = new HashMap<XMLWidget, Map<Integer, XMLWidget>>();
          ...
          if (values.item(a).getNodeName().trim().equals("location")) {
            ...
            for (int b = 0; b < subvalues.getLength(); b++) {
              ...
              if (nodename.equals("location")) {
                ...
                subwidget.setHeight(Integer.parseInt(subvalues.item(b).getChildNodes().item(0)
                    .getNodeValue()));
              } else if (nodename.equals("width")) {
                subwidget.setWidth(Integer.parseInt(subvalues.item(b).getChildNodes().item(0)
                    .getNodeValue()));
              }
            }
            System.out.println(
  ...
  public boolean containsWidgetOfName(LWSubsystem subsystem, String name) {
    ...
    for (XMLWidget w : subsystems.keySet()) {
      if (((LWSubsystem) w.convertToDisplayElement()).getFieldName().equals(subsystem
          .getFieldName())) {
