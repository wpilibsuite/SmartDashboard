...
public class GlassPane extends JPanel {
  ...
  private void defineBounds() {
    ...
    if (selectedElement.isResizable()) {
      ...
      area = new Rectangle(
          sb.x + sb.width - xbuffer, sb.y + ybuffer, 2 * xbuffer, sb.height - 2 * ybuffer);
      areas.put(SwingConstants.EAST, area);

      area = new Rectangle(
          sb.x + sb.width - xbuffer, sb.y + sb.height - ybuffer, 2 * xbuffer, 2 * ybuffer);
      areas.put(SwingConstants.SOUTH_EAST, area);

      area = new Rectangle(
          sb.x + xbuffer, sb.y + sb.height - ybuffer, sb.width - 2 * xbuffer, 2 * ybuffer);
      ...
      area = new Rectangle(
          sb.x + xbuffer, sb.y + ybuffer, sb.width - 2 * xbuffer, sb.height - 2 * ybuffer);
  ...
  private class GlassMouseListener extends MouseAdapter {
    ...
    private int adjustW(int value) {
      return adjust(value,
          dragStartBounds.x + dragStartBounds.width, frame.getPrefs().grid_widths.getValue());
    }

    private int adjustH(int value) {
      return adjust(value,
          dragStartBounds.y + dragStartBounds.height, frame.getPrefs().grid_heights.getValue());
    }
      ...
      if (selectedElement != null && dragType != -1) {
        ...
        switch (dragType) {
          ...
          case SwingConstants.CENTER:
            ...
            int leading = adjust(
                e.getPoint().x - dragStartPoint.x, dragStartBounds.x, prefs.grid_widths.getValue());
            int trailing = adjust(
                e.getPoint().x - dragStartPoint.x,
                dragStartBounds.x + dragStartBounds.width, prefs.grid_widths.getValue());
            dx = Math.abs(leading) < Math.abs(trailing) ? leading : trailing;
            leading = adjust(e.getPoint().y
                - dragStartPoint.y, dragStartBounds.y, prefs.grid_heights.getValue());
            trailing = adjust(
                e.getPoint().y - dragStartPoint.y,
