...
public class WebcamViewerExtension extends StaticWidget implements Runnable {
      ...
      try {
        ...
        while (!Thread.interrupted() &&
            framesize == (Integer) sizeProperty.getValue()) {
                    /* Each frame has a header with 4 magic bytes and the number of bytes in the
                    image */
          ...
          assert Arrays.equals(magic, MAGIC_NUMBERS);
                    ...
                    /* Get the image data itself, and make sure that it's a valid JPEG image (it
                    starts with
  ...
  protected void paintComponent(Graphics g) {
    ...
    synchronized (this.frameMutex) {
            /* Adjust the image size and location depending on how the aspect ratio matches up
